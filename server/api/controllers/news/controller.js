...
export class Controller {
    ...
    async getRecent(req,res){
        ...
        const sortedByTime = await newsModel.find({category : category}).sort([["postTime" , -1]]).limit(number)
        ...
        try{
            ...
            for(let i = 0; i < sortedByTimeCopy.length - 1; i++){
                ...
                for(let j = i; j < sortedByTimeCopy.length;j++){
                    ...
                    if(sortedByTimeCopy[j].viewer.length > sortedByTimeCopy[i].viewer.length){
                        // console.log("swap")
    ...
    async search(req,res){
        ...
        if(sortBy === "claps"){
            try{
                const filteredByTag = tags.length > 0 ? await newsModel.find({ "tags" : { $all : tags } , category : category }) : await newsModel.find({category : category})
                ...
                for(let i = 0; i < filteredByTagCopy.length; i++){
                    ...
                    for(let j = i; j < filteredByTagCopy.length;j++){
                        ...
                        if(filteredByTagCopy[j].viewer.length > filteredByTagCopy[i].viewer.length){
                            // console.log("swap")
            ...
            try{
                filteredByTagAndTime = tags.length > 0 ? await newsModel.find({ "tags" : { $all : tags }, category : category }).sort([["postTime" , -1]]) : await newsModel.find({category : category}).sort([["postTime" , -1]]) ;
